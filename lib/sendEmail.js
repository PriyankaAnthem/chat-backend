...
export async function sendUnansweredEmail(question) {
  ...
  try {
    console.log("💾 Attempting DB save for:", question);
  ...
  try {
    ...
    const adminUrl = `${process.env.NEXT_PUBLIC_APP_URL?.split(",")[0].trim()}/admin/unanswered${
      questionId ? `?highlight=${questionId}` : ""
    }`;

    console.log("📧 Sending mail to diljeet@antheminfotech.com...");
    const info = await transporter.sendMail({
      from: `Chatbot <${process.env.EMAIL_USER}>`,
      to: "diljeet@antheminfotech.com",
